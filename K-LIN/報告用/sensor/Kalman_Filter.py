...
x = numpy.random.normal(1,0.1,size=sz) # truth value (typo in example at top of p. 13 calls this z)真实值  
...
pylab.figure()  
pylab.plot(z,'k+',label='Infrared sensor')     #观测值  
pylab.plot(xhat,'b-',label='a posteriori estimation')  #滤波估计值  
pylab.plot(x,color='g',label='Ultrasonic sensor')    #真实值  
pylab.legend()  
pylab.xlabel('time')  
...
pylab.plot(valid_iter,Pminus[valid_iter],label='a posteriori error estimate')  
pylab.xlabel('time')  
pylab.ylabel('distance')  
